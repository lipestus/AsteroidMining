            ...
            if (breakOnCollision)
            {
                switch (collisionSettings.collisionMode)
                {
                    ...
                    case Enums.CollisionMode.CheckAllowedTags:
                        if (collisionSettings.allowedTags.Contains(other.gameObject.tag))
                            Break();
                        break;
                    case Enums.CollisionMode.CheckIgnoredTags:
                        if (!collisionSettings.ignoredTags.Contains(other.gameObject.tag))
                            Break();
                        break;
                    case Enums.CollisionMode.CheckAllowedAndIgnoredTags:
                        if (collisionSettings.allowedTags.Contains(other.gameObject.tag)
                            && !collisionSettings.ignoredTags.Contains(other.gameObject.tag))
                            Break();
                        break;
